...
def save_parsed_file_to_output_directory(parsed_file, outputdir, navigation_template):
    # Save header
    save_new_file_with_title(outputdir + "/" + parsed_file.header_section.get_markdown_link(),
                             parsed_file.header_section.title,
                             parsed_file.header_section.lines)
    # Save each section
    ...
    if args.navigation_template != "DEFAULT":
        try:
            with open(args.navigation_template, 'r') as template_file:
                navigation_template = template_file.read().replace('\n', '')
